    ...
    <div class="card-body">
        <form action="{{ route("admin.support.update", [$support->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('user_id') ? 'has-error' : '' }} mt-3">
                ...
                <option value="">Select</option>
                    @foreach($user as $id => $item)
                        <option value="{{ $id }}" @if($support->user_id==$id) selected  @endif>{{ $item->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('source') ? 'has-error' : '' }} mt-3">
                ...
                <option value="">Select</option>
                  <option value="website" @if($support->source=="website") selected  @endif>Website</option>
                  <option value="Whatsapp" @if($support->source=="Whatsapp") selected  @endif>Whatsapp</option>
                  <option value="email" @if($support->source=="email") selected  @endif>Email</option>
                  <option value="application" @if($support->source=="application") selected  @endif>Application</option>
                  </select>
